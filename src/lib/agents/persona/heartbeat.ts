...
import { postMessage } from "@/lib/agents/runtime/slack-manager";
import { getGoalState, updateGoal } from "@/lib/agents/persona/goal-manager";
...
async function buildHeartbeatContext(slug: string): Promise<HeartbeatContext | null> {
  ...
  let tasksContext = "";
  try {
    const { getTasksForAgent } = await import("@/lib/agents/inbox/task-inbox");
...
async function processHeartbeatOutput(
  ...
  if (memoryMatch) {
    ...
    for (const match of taskMatches) {
      const { createTask } = await import("@/lib/agents/inbox/task-inbox");
    ...
    for (const match of taskCompleteMatches) {
      const { updateTask } = await import("@/lib/agents/inbox/task-inbox");
...
export async function runQuickResponse(
  ...
  let recentMessages = "";
  try {
    const { getMessages } = await import("@/lib/agents/runtime/slack-manager");
