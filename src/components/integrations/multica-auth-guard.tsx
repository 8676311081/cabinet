...
import { multicaFetch, primeElectronMulticaSession } from "@/lib/electron-desktop";

async function fetchOwnerSession() {
  const [meRes, workspacesRes] = await Promise.all([
    multicaFetch("/me", { cache: "no-store" }),
    multicaFetch("/workspaces", { cache: "no-store" }),
  ]);

  if (!meRes.ok) {
    throw new Error(`Failed to load Multica user (${meRes.status})`);
  }
  if (!workspacesRes.ok) {
    throw new Error(`Failed to load Multica workspaces (${workspacesRes.status})`);
  }
...
export function MulticaAuthGuard({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function autoLogin() {
      api.setToken(primeElectronMulticaSession());

      for (let attempt = 1; attempt <= AUTO_LOGIN_MAX_RETRIES; attempt += 1) {
        try {
          const { user: me, workspaces: wsList } = await fetchOwnerSession();
          if (disposed) return;
          ...
          console.error("[multica] embedded auto-login failed", {
            attempt,
